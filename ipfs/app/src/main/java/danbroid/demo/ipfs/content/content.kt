  ...
  rootMenu<MenuItemBuilder> {
    ...
    menu {
      ...
      onClick = {
        runCommand { ipfs ->
          ipfs.newRequest("cat")
            .withArgument("/ipns/QmfBFfV72Rw9Vm9wWr2HFARydNWt4ap2xvU8j7Uw2xzzDR").send().also {
...
/*          ipfs.newRequest("pubsub/sub").withArgument("poiqwe098123")
    ...
    menu {
      ...
      onClick = {
        ...
        runCommand { ipfs ->
          ipfs.newRequest("pubsub/pub").withArgument("poiqwe098123").withArgument("Hello!")
            .send()?.also {
              log.debug("received: ${it.decodeToString()}")
              withContext(Dispatchers.Main) {
                Toast.makeText(context, it.decodeToString(), Toast.LENGTH_SHORT).show()
    ...
    menu {
      ...
      onClick = {
        ...
        runCommand { ipfs ->
          httpClient.newCall(Request.Builder().url("http://192.168.1.2/config.json").build())
            .execute().body?.also {
